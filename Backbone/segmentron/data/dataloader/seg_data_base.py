...
class SegmentationDataset(object):
    ...
    def _sync_transform(self, img, mask, resize=False):
        ...
        if cfg.AUG.PERSPECTIVE:
            p = torchvision.transforms.RandomPerspective(distortion_scale=0.8, p=1, fill=-1)
            # img = torchvision.transforms.RandomPerspective(distortion_scale=0.8, p=1, fill=0)(img)
            # mask = torchvision.transforms.RandomPerspective(distortion_scale=0.8, p=1, fill=-1)(mask)
            img = p(img)
            mask = p(mask)
