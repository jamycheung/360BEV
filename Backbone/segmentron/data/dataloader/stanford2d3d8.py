...
class Stanford2d3d8Segmentation(SegmentationDataset):
    ...
    def _mask_transform(self, mask):
        target = self._map13to8(np.array(mask).astype('int32'))
...
if __name__ == '__main__':
    ...
    input_transform = transforms.Compose([transforms.ToTensor(), transforms.Normalize((.485, .456, .406), (.229, .224, .225)),])
     # Create Dataset
    trainset = Stanford2d3d8Segmentation(split='train', transform=input_transform)
