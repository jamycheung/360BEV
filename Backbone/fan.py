...
class FAN(nn.Module):
    ...
    ####fan_vit_tiny  embed_dim = 192, num_heads = 4
    ####fan_vit_small embed_dim = 384, num_heads = 8
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=192, depth=12,
                 num_heads=4, mlp_ratio=4., qkv_bias=True, drop_rate=0., attn_drop_rate=0., drop_path_rate=0.,
        ...
        self.linear_c4 = LinearMLP(input_dim=192, embed_dim=64)
        self.linear_c3 = LinearMLP(input_dim=192, embed_dim=64)
        self.linear_c2 = LinearMLP(input_dim=192, embed_dim=64)
        self.linear_c1 = LinearMLP(input_dim=192, embed_dim=64)
        self.linear_fuse = nn.Conv2d(4 * 64, 64, 1)  # 64
    ...
    def forward(self, x):
        ...
        _c4 = F.interpolate(_c4, size=(120, 160), mode='bilinear', align_corners=False)

        _c3 = self.linear_c3(c3).permute(0, 2, 1).reshape(n, -1, c3.shape[2], c3.shape[3])
        _c3 = F.interpolate(_c3, size=(120, 160), mode='bilinear', align_corners=False)

        _c2 = self.linear_c2(c2).permute(0, 2, 1).reshape(n, -1, c2.shape[2], c2.shape[3])
        _c2 = F.interpolate(_c2, size=(120, 160), mode='bilinear', align_corners=False)

        _c1 = self.linear_c1(c1).permute(0, 2, 1).reshape(n, -1, c1.shape[2], c1.shape[3])
        _c1 = F.interpolate(_c1, size=(120, 160), mode='bilinear', align_corners=False)
