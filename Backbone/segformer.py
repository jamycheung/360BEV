...
class OverlapPatchEmbed(nn.Module):
    ...
    def forward(self, x):
        ...
        # print('OverlapPatchEmbed0:',  x.shape)
        ...
        x = x.flatten(2).transpose(1, 2)
        x = self.norm(x)
        # print('OverlapPatchEmbed:',  self.H, self.W, H, W, x.shape)
...
class Segformer(nn.Module):
    def __init__(self, img_size=224, patch_size=4, in_chans=3, num_classes=13, embed_dims=[64, 128, 320, 512],
        ...
        self.linear_fuse = nn.Conv2d(4 * decoder_dim, 64, 1)  # 64
        # self.linear_output_4level = nn.Conv2d(768, 256, 1)
        
        # self.linear_fuse256= nn.Conv2d(4* decoder_dim, 256, 1) # 256
    ...
    def forward(self, x, is_feat=True):
        ...
        embeds, x = self.forward_features(x)
        # x = self.head(x)
        ...
        c1, c2, c3, c4 = x
        # print("c1-c4:", c1.size(), c2.size(), c3.size(), c4.size())
        ############## MLP decoder on C1-C4 ###########
        n, _, h, w = c4.shape
        h_out, w_out = c1.size()[2], c1.size()[3]

        _c4 = self.linear_c4(c4).permute(0, 2, 1).reshape(n, -1, c4.shape[2], c4.shape[3])
        # _c4 = self.linear_output_4level(_c4)
        ...
        # _c1 = self.linear_output_4level(_c1)
        
        _c = self.linear_fuse(torch.cat([_c4, _c3, _c2, _c1], dim=1))
        # _c  = self.linear_fuse256(torch.cat([_c4, _c3, _c2, _c1], dim=1))
...
class mit_b0_kd(Segformer):
    def __init__(self, **kwargs):
        super(mit_b0_kd, self).__init__(
            patch_size=4, embed_dims=[32, 64, 160, 256], num_heads=[1, 2, 5, 8], mlp_ratios=[4, 4, 4, 4],
            qkv_bias=True, norm_layer=partial(nn.LayerNorm, eps=1e-6), depths=[2, 2, 2, 2], sr_ratios=[8, 4, 2, 1],
            drop_rate=0.0, drop_path_rate=0.1, decoder_dim=256)


class mit_b1_kd(Segformer):
    def __init__(self, **kwargs):
        super(mit_b1_kd, self).__init__(
...
class mit_b4_kd(Segformer):
    def __init__(self, **kwargs):
        super(mit_b4_kd, self).__init__(
            patch_size=4, embed_dims=[64, 128, 320, 512], num_heads=[1, 2, 5, 8], mlp_ratios=[4, 4, 4, 4],
            qkv_bias=True, norm_layer=partial(nn.LayerNorm, eps=1e-6), depths=[3, 8, 27, 3], sr_ratios=[8, 4, 2, 1],
            drop_rate=0.0, drop_path_rate=0.1, decoder_dim=768)